...
%	a=linspace(-l,l,m);
%	for i=1:m
%		rp(1,1+m*(i-1):m+m*(i-1))=a+0.5*rand(1,m);
%		sp(1+m*(i-1):m+m*(i-1),1)=1.5*rand(m,1);		% Normally distributed
...
% Numerics
	...
	print(fig2,sprintf('output/%s/run%03d_%03d_rp.png',id,run,rep),'-dpng');
	print(fig3,sprintf('output/%s/run%03d_%03d_dyn.png',id,run,rep),'-dpng');
